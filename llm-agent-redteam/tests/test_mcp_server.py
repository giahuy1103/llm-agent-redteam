"""Unit tests for mock MCP tools logic (src/mcp_server.py).

Tests security enforcement, boundary controls, input validation, and forensic logging
completely offline without requiring Gemini API network connectivity.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. Tests for read_file
# ─────────────────────────────────────────────────────────────────────────────

def test_read_file_success(test_config: Config) -> None:
    """Verifies that an authorized file inside mock_files is read correctly."""
...
def test_read_file_path_traversal_blocked(test_config: Config) -> None:
    """Verifies that path traversal attacks (../) are intercepted and raise ToolExecutionError.

    The error message must not disclose real system paths.
    """
...
def test_read_file_missing_file_raises_error(test_config: Config) -> None:
    """Verifies that attempting to read a non-existent file raises a clear error."""
    with pytest.raises(ToolExecutionError) as exc_info:
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. Tests for send_email
# ─────────────────────────────────────────────────────────────────────────────

def test_send_email_valid_appends_jsonl_log(tmp_results_dir: Path) -> None:
    """Verifies that valid emails are formatted, confirmed, and logged to sent_emails_log.jsonl."""
...
def test_send_email_invalid_recipient_rejected(tmp_results_dir: Path) -> None:
    """Verifies that malformed email strings are rejected with ToolExecutionError."""
    invalid_addresses = [
        "not-an-email",
        "user@",
        "@domain.com",
...
# ─────────────────────────────────────────────────────────────────────────────
# 3. Tests for query_db
# ─────────────────────────────────────────────────────────────────────────────

def test_query_db_valid_select_returns_json(tmp_db_path: Path) -> None:
    """Verifies that a benign SELECT query executes and returns a JSON list of dictionaries."""
...
def test_query_db_rejects_non_select_statements(tmp_db_path: Path) -> None:
    """Verifies that non-SELECT statements (DROP, INSERT, UPDATE, DELETE) are rejected."""
    forbidden_queries = [
        "DROP TABLE users",
...
def test_query_db_blocks_semicolon_chaining_injection(tmp_db_path: Path) -> None:
    """Verifies that SQL injection attempts chaining commands via semicolon are blocked."""
    chained_injections = [
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. Tests for web_fetch
# ─────────────────────────────────────────────────────────────────────────────

def test_web_fetch_known_urls_return_mock_data() -> None:
    """Verifies that known URLs in MOCK_URL_RESPONSES return expected mock content."""
...
def test_web_fetch_unknown_url_returns_graceful_fallback() -> None:
    """Verifies that unknown URLs return the default mock message without raising an error."""
